...
misstestOutputContainer = "miss-test-yesterday-op"
...
# %% [markdown]
# #Note that this only accounts for Yesterday's Misses. There needs to be another script for Today's Misses
...
# %%
picked_in_range(df_FEZ_Picking_Data_YST,date_array()['yesterday'], date_array()['four days ago'])
...
def receiving_miss(today, four_days_ago):
    ...
    for uid in df_ez_gr_sales['UID']:
        ...
        # Check and convert 'rolling_qoh' to float
...
# %%
receiving_miss(date_array()['today'], date_array()['four days ago'])
...
#             debug.append(str(quantity-quantity_f_r)+ "BOM: "+ str(RM_master_bom))
        else:
            temp.append(0)
#             debug.append("not applicable")
    df_ez_gr_sales["Parent Needed"] = pd.Series(temp)
#     df_ez_gr_sales["Parent Needed_debug"] = pd.Series(debug)
...
def output():
    ...
    for uid in df_ez_gr_sales["UID"]:
        ...
        if miss_type == "Fulfillment" or miss_type == "Manufacturing" or miss_type == "Quarantine":
            #Append the columns needed for report
    ...
    uploadBlobClient = blobConnect.get_blob_client(container=misstestOutputContainer, blob="miss_test_op_yesterday.csv")
